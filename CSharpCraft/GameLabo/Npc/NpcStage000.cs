...
namespace GameLabo
{
    /// <summary>
    /// ステージ000専用のNPC制御クラス
    /// 主にスライム系NPCの生成・移動・当たり判定・復活を管理する
    /// </summary>
    public class NpcStage000 : NpcStageBase
    {
        /// <summary>
        /// ランダム処理用
        /// </summary>
                ...
                if (userid == StClass.UserID)
                {
                    ...
                    if (npc.Handle < 0)
                    {
                        switch (GetRand(1))
                        {
                            case 1:
                                {
                                    npc.ModelType = (int)Model.Model_CharaType.SlimeBes;
                                }
                                break;
                            default:
                                {
                                    npc.ModelType = (int)Model.Model_CharaType.Slime;
                                }
                                break;
                        }
                    ...
                    if (npc.HP > 0)
                    {
                        ...
                        if (npc.IsMoving)
                        {
                            ...
                            {
                                // 移動
                                float moveDist = 3f * StClass.loopTime;
                            ...
                            if (m.ArrowAlive == TRUE)
                            {
                                if (npc.HP > 0)
                                {
                                    ...
                                    if (hit.HitNum > 0)
                                    {
                                        ...
                                        m.HunterStatus = 2;
                                        m.ArrowAlive = FALSE;
                                        if (NpcInfo[i].ModelType == (int)Model.Model_CharaType.Slime)
                                        {
                                            m.PreyType = (int)Model.Model_CharaType.Slime;
                                            m.PreyScore = 1;
                                        }
                                        else
                                        {
                                            m.PreyType = (int)Model.Model_CharaType.SlimeBes;
                                            m.PreyScore = 5;
                                        }
                                        npc.HP = 0;
                                        npc.MoveTimer = 0;
        ...
        private void SetRandomTarget(ref ModelInfo npc, float radius)
        {
            ...
            if ((StClass.DAT.modelInfo[userid] != null) && (GetRand(100) < 80))
            {
